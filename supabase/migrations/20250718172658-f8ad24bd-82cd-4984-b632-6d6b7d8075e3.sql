-- Create enum for account status first
CREATE TYPE public.account_status AS ENUM ('pending', 'active', 'blocked', 'expired');

-- Add account status column with the correct type
ALTER TABLE public.users ADD COLUMN status public.account_status NOT NULL DEFAULT 'pending';
...
-- Update existing policies to consider account status
